...
export class DashboardComponent implements OnInit {
  ...
  startSearchVoice(target: 'events' | 'incidents' | 'logs' | 'assistant'): void {
    ...
    if (!SpeechRecognition) {
      this.errorMsg.set('La recherche vocale nécessite Chrome ou Edge avec l’autorisation du microphone.');
      return;
    }
    ...
    recognition.onerror = (event: any) => {
      this.activeVoiceSearch.set(null);
      this.activeRecognition = null;
      this.errorMsg.set(`Recherche vocale indisponible : ${event.error || 'erreur inconnue'}.`);
      this.cdr.detectChanges();
    };
    recognition.onend = () => {
      this.activeVoiceSearch.set(null);
